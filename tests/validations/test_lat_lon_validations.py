...
@pytest.mark.parametrize(
    ...
    [(-90.0, -90.0), (0.0, 0.0), (90.0, 90.0), (-91.0, ValueError), (91.0, ValueError)],
)
def test_latitude_validation(latitude, expected_result):
    if isinstance(expected_result, float):
        station = Station(
            station_id=1, station_name="Central", latitude=latitude, longitude=0.0
        )
...
def test_longitude_validation(longitude, expected_result):
    if isinstance(expected_result, float):
        station = Station(
            station_id=1, station_name="Central", latitude=0.0, longitude=longitude
        )
